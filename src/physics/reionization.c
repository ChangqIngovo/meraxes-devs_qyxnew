...
void malloc_reionization_grids()
{
   ...
   *                        [1e60 erg / code_time], populated each
   *                        snapshot in construct_baryon_grids().
  ...
  if (run_globals.params.Flag_PatchyReion) {
    ...
    if (run_globals.params.Flag_IncludeSpinTemp) {
      ...
      if (run_globals.params.physics.Flag_BHFeedback) {
        ...
        for (size_t ii = 0; ii < (size_t)slab_n_complex * 2 * run_globals.NstoreSnapshots_SFR; ii++)
...
void construct_baryon_grids(int snapshot, int local_ngals)
{
  ...
  enum property
  {
     ...
     * THIS ADDITION IS MADE TO ADD THE X-RAY CONTRIBUTION BY AGN
     * Physical motivation:
     *   prop_bh_xray_emissivity deposits gal->BHXrayEmissivity (the
     *   obscuration-corrected AGN X-ray energy emitted per galaxy per
     *   snapshot, computed in blackhole_feedback.c) onto the spatial
     *   grid using the same MPI slab-reduction as all other properties.
     *   This is the AGN analogue of prop_sfr for stellar X-ray sources.
     */
  ...
  for (int prop = prop_stellar; prop <= prop_sfr; prop++) {
    ...
    /*
     * THIS ADDITION IS MADE TO ADD THE X-RAY CONTRIBUTION BY AGN
     * Skip the AGN emissivity property if SpinTemp is off (no heating
    ...
    if (prop == prop_bh_xray_emissivity) {
      ...
      if (!run_globals.params.physics.Flag_BHFeedback) continue;
      if (bh_xray_grid == NULL) continue;
    }
    ...
    for (int i_r = 0; i_r < run_globals.mpi_size; i_r++) {
      ...
      if (local_ngals != 0)
        ...
        while (((i_gal - skipped_gals) < local_ngals) && (galaxy_to_slab_map[i_gal].slab_ind == i_r)) {
          ...
          switch (prop) {
             ...
             * THIS ADDITION IS MADE TO ADD THE X-RAY CONTRIBUTION BY AGN
             * Physical motivation:
             *   gal->BHXrayEmissivity is the total AGN X-ray energy
             *   emitted by this galaxy's black hole over the current
             *   snapshot, in internal Meraxes units [1e60 erg]. It is
             *   already corrected for obscuration (NH distribution +
             *   band-averaged transmission) by apply_xray_obscuration()
             *   in blackhole_feedback.c.
             *
             *   We accumulate it per cell exactly as gal->Sfr is
      ...
      if (run_globals.mpi_rank == i_r)
        ...
        switch (prop) {
           ...
           *   snapshot. We store this raw value (units: [1e60 erg]) into:
           *
